...
# Note that fasttrib_points() uses exactextractr::exact_extract() which uses 32-bit precision,
# so there may be some small differences in the values, but they should be very close.
# fastrib_points outputs align with Hmisc weighted mean, variance and sd calculations
all.equal(attr1, attr2)
...
dw_time <- system.time(
  Processed_HW <- prep_weights(
    input = hydro_out,
    output_filename = file.path(tempdir(), "Processed_HW_out.gpkg"),
...
fasttrib_points_time_big <- system.time(
  final_attributes_all <- fasttrib_points(
    ...
    #iDW_file =  file.path(tempdir(),"Processed_HW_out.gpkg"), # to use previously calculated weights
    store_iDW = T,
