...
tm_shape(hydro_out$subbasins) +
  ...
  tm_dots(
    col_alpha = 1,
    fill = "group",
    fill.scale = tm_scale_categorical(values = "hcl.dark2"),
    size = 0.75
  ) +
  tm_layout(legend.outside = TRUE)
```
...
***ihydro*** provides functions for calculating weighted spatial summaries: `prep_weights()` and `fasttrib_points()`. 
...
fasttrib_points_time <- system.time(
  final_attributes_all <- fasttrib_points(
    ...
    loi_file = output_filename_loi,
    loi_cols = NULL,
    iDW_file =  as_ihydro(file.path(tempdir(),"Processed_HW_out.gpkg")), # use previously calculated weights
    store_iDW = T,
...
paste0("prep_weights() took ",round(dw_time[3] / 60, 2) ," minutes with ",future::nbrOfWorkers()," cores.")

paste0("fasttrib_points() took ",round(fasttrib_points_time[3] / 60, 2) ," minutes with ",future::nbrOfWorkers()," cores.")
