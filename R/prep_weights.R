...
#' )
#'
#' lookup_tbl <- ihydro::read_ihydro(ihydro_weights,"target_o_meta")
...
resolve_available_weights <- function(
  ...
  if ("target_o_meta" %in% lyrs$layer_name) {
    existing_meta <- read_ihydro(output_filename, "target_o_meta") |>
    ...
    # full_join was introducing rows from target_o_meta for link_ids NOT in
...
compute_point_weights <- function(
  ...
  target_o_meta <- NULL
  if ("target_o_meta" %in% lyrs$layer_name) {
    target_o_meta <- read_ihydro(output_filename, "target_o_meta")
    # Filter by target_o_type if the column exists
    if ("target_o_type" %in% colnames(target_o_meta)) {
      target_o_meta <- dplyr::filter(
        target_o_meta,
        target_o_type == !!target_o_type
      )
    }
    target_o_meta <- target_o_meta |>
      dplyr::filter(
